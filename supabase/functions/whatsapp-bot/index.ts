      ...
      case "IDLE":
        if (text) {
          ...
          if (intent === "REPORTE") {
            ...
            await sendMessage(
              chatId,
              "📍 Para decirte cómo está tu zona, <b>envíame tu ubicación</b> usando el clip 📎 de WhatsApp (Ubicación)."
            );
          } else {
            await sendMessage(
              chatId,
      ...
      case "ESPERANDO_UBICACION_REPORTE":
        if (location) {
          ...
          if (session.intentos_fallidos >= 3) {
            ...
            await sendMessage(
              chatId,
              `❌ No reconozco esa ubicación. Tienes que usar la herramienta de adjuntar de WhatsApp (Ubicación). (Intento ${session.intentos_fallidos}/3)`
            );
          }
        }
        break;
